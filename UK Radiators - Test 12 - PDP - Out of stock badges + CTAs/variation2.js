...
(function () {
  try {
    ...
    let EmailField = `<div data-popmechanic-embed="61090"></div>`;
    ...
    function init() {
      if (!document.querySelector('body').classList.contains(variation_name)) {
        ...
        waitForElement('.mobileFlex:has(.pdp-add-to-cart)', function () {
          if (!document.querySelector('[data-popmechanic-embed="61090"]')) {
            document.querySelector('.mobileFlex:has(.pdp-add-to-cart)').insertAdjacentHTML('afterend', EmailField)
          }
        })
